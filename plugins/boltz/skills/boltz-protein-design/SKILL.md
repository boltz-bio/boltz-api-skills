...
4. **If the user wants antibody (Fab) or nanobody, STOP before authoring the payload and read [references/antibody-nanobody-design.md](references/antibody-nanobody-design.md).** Antibody and nanobody design requires a scaffold the user picks — do NOT silently default to a packaged scaffold. Ask the user first; only after they confirm a choice (their own CIF + CDR indices, or one of our 3 Fab / 3 nanobody packaged scaffolds) do you proceed to `estimate-cost`.
