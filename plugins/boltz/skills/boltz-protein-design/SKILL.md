...
5. Pick `modality` for `structure_template` and `no_template`. It is a **generation-config switch only**, not a scaffold loader. **If the user already named the modality, take it as given — don't ask again.** Do not include `modality` on `boltz_curated`; use `binder` instead. Options:
   - `custom_protein` — general de novo protein binder, including "miniprotein" or generic "protein binder" requests. Cys allowed. Includes a design-folding step and the largest-hydrophobic-patch filter. Use this for `protein-anything`, `protein-small_molecule`, and `protein-redesign` workflows from boltzgen.
