...
- **Spending gate — estimate first, auto-submit only below $1.00.** `start` spends real money. Run `estimate-cost`, show the `estimated_cost_usd` it returns (summed for a batch), and submit without asking only when the total is less than $1.00. For $1.00 or more, wait for the user to say go. A plan you already described or an earlier phase's approval is not authorization for a $1.00+ `start`. This holds **even when tool calls are pre-approved** (accept-edits / auto-accept / bypass modes) — there you are the only cost gate. Never quote or assume a dollar figure you didn't get from `estimate-cost` (cost model: [`## Cost`](references/api.md) in api.md).
