...
`python3` with `gemmi`, `numpy`, and `scipy`. **Probe the active interpreter
...
python3 -c "import gemmi, numpy, scipy" && echo DEPS_OK
```
...
2. **Disorder cutout** — if the target has internal disordered/loopy regions,
   ...
   ```
   It reports internal coil stretches longer than `--min-loop` (default 10) and
   prints the kept `crop_residues` (0-based) with those stretches removed. This
   is the weakest axis — there is no perfect internal-disorder detector — so
   treat the suggestion as a hint and confirm it looks reasonable.
...
2. After download, cluster the top designs' contact footprints into candidate
   sites:
   ```bash
   python3 scripts/scan_sites.py <run-dir> --top 20 --cutoff 6 --jaccard 0.25
   ```
   It computes each top design's all-atom footprint on the target (default 6 Å),
   greedily clusters footprints by Jaccard > 0.25, and prints a consensus site
   (0-based API indices) per cluster — the residues contacted by ≥2 designs in
   the cluster.
