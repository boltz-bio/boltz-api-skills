...
- `binder_specification` (required) — discriminated union: `structure_template` or `no_template`.
...
`modality` lives on `binder_specification` and is **required** in both variants. It is a **generation-config switch** — it changes inverse-folding rules, the design-folding step, and post-filters. **It does NOT auto-load any scaffold**: for `antibody` / `nanobody`, you still must supply your own scaffold structure via `binder_specification.structure_template` (or sequence components via `no_template`). The API will not fetch antibody framework geometry for you.

API vocabulary (all four are valid for both `structure_template` and `no_template`):
...
| boltzgen `--protocol` | API `modality` | Notes |
|---|---|---|
| `protein-anything` | `custom_protein` | Most general path. |
| `peptide-anything` | `peptide` | Add `cyclic: true` on the designed entity for cyclic peptides. |
| `protein-small_molecule` | `custom_protein` | Put the small molecule in `target.no_template.entities` as `ligand_ccd` or `ligand_smiles`. The API does **not** expose a separate small-molecule binding-affinity head — score downstream from output metrics. |
| `antibody-anything` | `antibody` | Provide the Fab framework as `binder_specification.structure_template` and mark the CDR loops with `design_motifs`. |
| `nanobody-anything` | `nanobody` | Provide the VHH framework as `binder_specification.structure_template`. |
| `protein-redesign` | (no dedicated modality) | Use `binder_specification.structure_template` with `design_motifs` covering the regions to redesign; pick `custom_protein` as the modality. |

If the user asks for "an antibody" or "a nanobody" without supplying a scaffold, push back and ask for one (CIF / PDB) — the API will not invent it.
...
Use when generating purely from sequence components + the DSL (no scaffold structure).
...
Optional, applies to both `binder_specification` variants.
...
target:
  ...
  entities:
    ...
    - type: ligand_ccd
      ...
      value: "SAH"
  epitope_residues:
    A: [42, 43, 44]                   # optional; per-chain, 0-based
