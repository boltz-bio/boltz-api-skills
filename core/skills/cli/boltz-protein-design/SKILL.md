...
- **Spending gate — get explicit confirmation before every `start`.** `start` spends real money, so never run it without the user's fresh go-ahead for *that* submission. A plan you already described, the user's approval of an earlier phase, and a cost you judge "small" or "trivial" are **not** authorization — a $1 run still needs a yes. Run `estimate-cost`, show the exact `estimated_cost_usd` (summed when submitting a batch), and wait for the user to say go. **This holds even when tool calls are pre-approved** (accept-edits / auto-accept / bypass-permission modes): there you are the *only* cost gate, so still stop and ask before submitting. Never spend on an assumed rate; cost scales with total complex length (target + binder) × count, so always quote the figure from `estimate-cost`.
