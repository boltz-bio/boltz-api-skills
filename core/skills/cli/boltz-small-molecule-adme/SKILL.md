...
3. Author the payload YAML or JSON, run `estimate-cost`, show the USD cost, and submit without asking only when `estimated_cost_usd` is less than $1.00; for $1.00 or more, wait for explicit confirmation. ADME is priced at **$0.01 per molecule** (size-independent); `estimate-cost` returns the authoritative total — always quote it.
...
- Run `estimate-cost` and show the USD total before submitting. Submit without asking only when `estimated_cost_usd` is less than $1.00; for $1.00 or more, wait for explicit confirmation. ADME is $0.01/molecule (size-independent); `estimate-cost` returns the authoritative total — always use it.
