...
- After the background/session starts, do not manually wait on it or run ad hoc polling loops. In Codex, schedule a same-thread heartbeat automation that checks `download-status` periodically, posts only material status changes or terminal completion/failure, and stops once terminal. In non-Codex hosts, use the host's native background notification mechanism when available; otherwise report the job ID, run name, output directory, and the command needed to check `download-status`.
...
- Prefer `download-status` for local checkpoint state. In Codex, use heartbeat automation for automatic follow-up and poll the saved session with an empty `write_stdin` only for interactive, user-requested progress checks. Don't loop `retrieve` unless the user wants fresh remote status.
