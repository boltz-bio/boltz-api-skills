...
7. When done, rank results by `optimization_score` descending (fallback `binding_confidence`), report the top 5–10 hits with `smiles` + key metrics, and point the user at `$ROOT/$RUN_NAME/results/`.
