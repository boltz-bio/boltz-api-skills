...
7. When done, rank via `list-results` (currently not exposed as a dedicated MCP tool — shell out with `boltz-api small-molecule:library-screen list-results --id "$JOB_ID" --format jsonl`). **Do not** rank from the per-hit `metrics.json` files on disk: they carry `external_id: null` / `smiles: null`, so you can't map scores back to your input library without `list-results`. Sort by `optimization_score` descending (fallback `binding_confidence`), report the top 5–10 hits with `smiles` + key metrics, and point the user at `$ROOT/$RUN_NAME/results/`.
