...
def parse_protein_library(value: str) -> list[dict[str, Any]]:
    ...
    if suffix in {".fasta", ".fa", ".faa"}:
        ...
        for line in p.read_text().splitlines():
            ...
            if line.startswith(">"):
                if cur_seq:
                    rec: dict[str, Any] = {"entities": [{"type": "protein", "value": "".join(cur_seq), "chain_ids": ["A"]}]}
                    if cur_id:
                        rec["id"] = cur_id
                    out.append(rec)
        ...
        if cur_seq:
            rec = {"entities": [{"type": "protein", "value": "".join(cur_seq), "chain_ids": ["A"]}]}
            if cur_id:
                rec["id"] = cur_id
            out.append(rec)
        return out
...
def build_kwargs(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if config.get("proteins"):
        proteins.extend(config["proteins"])
    if args.proteins:
    ...
    kwargs: dict[str, Any] = {"proteins": proteins, "target": target}
