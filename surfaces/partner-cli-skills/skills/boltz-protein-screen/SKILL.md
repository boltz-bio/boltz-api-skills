...
6. Launch `download-results` as a long-running/background command in whatever mode the host agent harness provides. After launching it, report the job ID, run name, and output directory, then end the turn immediately. Do not wait on the background job unless the user explicitly asks for progress.
...
- Run `download-results` through the host harness's long-running/background command facility. After it starts, do not wait on or poll it. Report the job ID, run name, and output directory and let the harness notify the user when the background command completes. `--poll-interval-seconds 30` is a reasonable default.
