...
Covers the `small-molecule:library-screen` endpoint. Prefer a single merged top-level `--input` payload for `start`. Field names below are **API body field names**, not CLI flag names.
...
Optional. Molecules failing any filter are skipped before scoring.
