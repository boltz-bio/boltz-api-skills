...
test("Skills preview does not install files and selection installs only requested skills", { timeout: 60000 }, async (t) => {
  const project = await mkdtemp(path.join(tmpdir(), "boltz-skills-selection-"));
  ...
  const preview = await exec(process.execPath, [cli, "add", repoRoot, "--list"], options);
  for (const name of await readdir(skillsRoot)) assert.ok(preview.stdout.includes(name), `Preview omits ${name}`);
  assert.deepEqual(await readdir(project), [], "Preview must not create an installation or lockfile");

  const selected = ["boltz-cli-setup", "boltz-protein-design"];
  await exec(process.execPath, [cli, "add", repoRoot, "--skill", ...selected, "--agent", "gemini-cli", "--yes"], options);
  const installed = path.join(project, ".agents/skills");
  assert.deepEqual((await readdir(installed)).sort(), selected);
  for (const name of selected) await assertSameFiles(path.join(skillsRoot, name), path.join(installed, name));
  const lock = JSON.parse(await readFile(path.join(project, "skills-lock.json"), "utf8"));
  assert.deepEqual(Object.keys(lock.skills).sort(), selected);
});
